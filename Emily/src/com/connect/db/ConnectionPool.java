...
public class ConnectionPool {
    ...
    private String testTable = "test"; // 测试连接是否可用的测试表名，默认没有测试表
    ...
    // 初始时为 null它中存放的对象为 PooledConnection 型
    ...
    /**
     * 创建由 numConnections 指定数目的多个数据库PooledConnection连接 , 并把这些连接
     * 放入 connections 向量中
    ...
    private void createConnections(int numConnections) throws SQLException {
        ...
        for (int x = 0; x < numConnections; x++) {
            ...
            System.out.println("数据库连接个数"+connections.size());
        }
    }

    /**
     * 创建一个新的数据库Connection连接并返回它
    ...
    private Connection newConnection() throws SQLException {
        ...
        i++;
        System.out.println("第"+i+" 数据库连接己创建 ......");
     ...
     * 本函数则根据 incrementalConnections 设置的值创建几个数据库连接，并放入连接池中。\
    ...
    private PooledConnection findFreeConnection() throws SQLException {
        //通过轮询的方式来查找是否有可用的数据库连接对象获得连接池向量中所有的对象
     ...
     * @throws SQLException
     */
    public synchronized void refreHshConnections() throws SQLException {
        // 确保连接池己创新存在
        if (connections == null) {
    ...
    class PooledConnection {
        ...
        public int preUpdate(String sql,Object... obj) throws SQLException {
